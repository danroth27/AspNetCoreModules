...
{
    public class ModuleStartupLoader
    {
        public static ModuleStartupMethods LoadMethods(IServiceProvider hostingServiceProvider, Type startupType, string environmentName)
        {
            var configureMethod = FindConfigureDelegate(startupType, environmentName);
            var servicesMethod = FindConfigureServicesDelegate(startupType, environmentName);
            var configureContainerMethod = FindConfigureContainerDelegate(startupType, environmentName);
            var sharedServicesMethod = FindConfigureSharedServicesDelegate(startupType, environmentName);
            
            object instance = null;
            if (!configureMethod.MethodInfo.IsStatic 
                || (servicesMethod != null && !servicesMethod.MethodInfo.IsStatic)
                || (sharedServicesMethod != null && !sharedServicesMethod.MethodInfo.IsStatic))
            {
            ...
            return new ModuleStartupMethods(configureMethod.Build(instance), configureServices, sharedServicesMethod.Build(instance));
        }
        ...
        private static ConfigureServicesBuilder FindConfigureSharedServicesDelegate(Type startupType, string environmentName)
        {
            var servicesMethod = FindMethod(startupType, "ConfigureShared{0}Services", environmentName, typeof(void), required: false);
            return new ConfigureServicesBuilder(servicesMethod);
        }
